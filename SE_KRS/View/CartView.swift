...
struct CartView: View {
    ...
    private var cartHeader: some View {
        if let firstItem = viewModel.activeCart.first {
            ...
            HStack {
                ...
                Text(restaurantName).font(.headline).fontWeight(.semibold)
                Spacer()
            }
        }
    }
    ...
    private var emptyCartView: some View {
        VStack {
            ...
            Text("Keranjang Anda kosong")
                .font(.title3).fontWeight(.semibold).padding(.top, 8)
    ...
    private var orderSummaryCard: some View {
        VStack(alignment: .leading, spacing: 12) {
            Text("Ringkasan Pesanan").font(.headline).fontWeight(.semibold)
            ...
            HStack {
                Text("Total Pembayaran").fontWeight(.bold)
                Spacer()
                Text("Rp\(String(format: "%.0f", viewModel.currentCartTotalPayable))").fontWeight(.bold)
            }.font(.subheadline)
        }
    ...
    private var paymentMethodCard: some View {
        VStack(alignment: .leading, spacing: 12) {
            Text("Metode Pembayaran").font(.headline).fontWeight(.semibold)
            ...
            if selectedPaymentMethod == .balance {
                HStack {
                    ...
                    if let userId = userSession.currentUserId, let balance = DummyDataStore.shared.userBalances[userId] {
                        ...
                        Text("Rp0").foregroundColor(.gray)
                    }
                }
                .font(.footnote).padding(8).background(Color.blue.opacity(0.1)).cornerRadius(8)
            }
        }
        .padding().background(Color.white).cornerRadius(12)
        .shadow(color: .black.opacity(0.05), radius: 5, y: 3)
    ...
    private func summaryRow(label: String, value: Double, color: Color = .primary) -> some View {
        HStack {
            ...
            Spacer()
            Text("Rp\(String(format: "%.0f", value))").fontWeight(.medium).foregroundColor(color)
        }.font(.subheadline)
    }
}
